    ...
    @if (currentUser(); as user) {
      <div class="flex h-screen bg-gray-100 font-sans text-gray-800">
        ...
        <aside 
          ...
          [class.md:w-64]="true"
        >
          <div class="h-16 flex items-center justify-center text-2xl font-bold flex-shrink-0 px-4">
          ...
          <nav class="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
            ...
            @if(user.role === 'client') {
              <div class="px-4 pt-4">
                <button (click)="openNewRequestForm()" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center">
          ...
          <div class="p-4 border-t border-gray-700">
            <div class="flex items-center">
              <img [src]="user.avatar_url || 'https://i.pravatar.cc/40'" alt="User Avatar" class="w-10 h-10 rounded-full object-cover">
              <div class="ml-3 truncate">
                <p class="text-sm font-semibold">{{ user.name }}</p>
                <p class="text-xs text-gray-400">{{ user.role | i18n }}</p>
              </div>
            </div>
            <button (click)="handleLogout()" class="w-full mt-4 text-left flex items-center px-4 py-2 text-sm rounded-md hover:bg-gray-700">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          <header class="bg-white shadow-sm p-4 flex justify-between items-center z-10 flex-shrink-0">
            <button (click)="isSidebarOpen.set(!isSidebarOpen())" class="text-gray-600 md:hidden">
              <i class="fas fa-bars text-xl"></i>
            </button>
            <h1 class="text-xl font-semibold text-gray-700 hidden md:block">{{ currentNav() | i18n }}</h1>
            <div class="flex items-center space-x-4">
              <app-language-switcher theme="light" />
              <button (click)="isNotificationCenterOpen.set(true)" class="relative text-gray-600 hover:text-indigo-600">
      ...
      @if (isNewRequestFormOpen()) {
        <div class="modal-backdrop" (click)="closeModal()">
          <div class="modal-content" (click)="$event.stopPropagation()">
            <app-service-request-form [user]="user" (close)="closeModal()" (formSubmitted)="handleFormSubmitted($event)" />
          </div>
        </div>
      }
      @if (isNotificationCenterOpen()) {
        <div class="modal-backdrop-right" (click)="closeModal()">
          <app-notification-center (close)="closeModal()" (click)="$event.stopPropagation()" />
        </div>
      }
      @if (isChatOpen() && selectedRequest()) {
        <div class="modal-backdrop" (click)="closeModal()">
          <div class="modal-content max-w-2xl h-3/4" (click)="$event.stopPropagation()">
             <app-chat [currentUser]="user" [serviceRequest]="selectedRequest()!" (close)="closeModal()" />
          </div>
        </div>
      }
      @if (isSchedulerOpen() && selectedRequest()) {
        <div class="modal-backdrop" (click)="closeModal()">
          <div class="modal-content" (click)="$event.stopPropagation()">
            <app-scheduler [serviceRequest]="selectedRequest()!" (close)="closeModal()" (appointmentConfirmed)="handleScheduleConfirmed($event)" />
          </div>
        </div>
      }
...
export class AppComponent {
  ...
  navItems = computed(() => {
    ...
    const items: { id: Nav; labelKey: string; icon: string; }[] = [
      { id: 'dashboard', labelKey: 'dashboard', icon: 'fa-solid fa-table-columns' },
      { id: 'schedule', labelKey: 'schedule', icon: 'fa-solid fa-calendar-days' },
      { id: 'search', labelKey: 'search', icon: 'fa-solid fa-magnifying-glass' },
      { id: 'profile', labelKey: 'profile', icon: 'fa-solid fa-user' },
    ];
    if (userRole === 'admin') {
      items.push({ id: 'admin', labelKey: 'admin', icon: 'fa-solid fa-user-shield' });
    }
    return items;
  });
  ...
  // --- View Navigation ---
  showLogin() { this.view.set('login'); }
  showRegister() { this.view.set('register'); }
  showLanding() { this.view.set('landing'); }
  navigate(nav: Nav) { 
  ...
  handleVerification(code: string) {
    this.authService.verifyOtp(this.emailForVerification(), code).then(() => {
        this.notificationService.addNotification('Verification successful! You can now log in.');
        this.view.set('login');
    });
  }

  handleResendVerification() {
    this.notificationService.addNotification('A new verification code has been sent.');
  ...
  handleScheduleConfirmed(event: { requestId: number, professionalId: number, date: Date }) {
    this.dataService.scheduleServiceRequest(event.requestId, event.professionalId, event.date);
    this.closeModal();
  }

  handleFormSubmitted(payload: ServiceRequestPayload) {
  ...
  handlePayment(request: ServiceRequest) {
    ...
    this.notificationService.addNotification(`Payment for request #${request.id} processed.`);
  }
}