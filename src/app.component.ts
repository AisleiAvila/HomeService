        ...
        <aside class="w-64 bg-gray-800 text-white flex flex-col">
          ...
          <nav class="flex-1 px-4 py-6 space-y-2">
            @for (view of availableViews(); track view.id) {
              <a 
                href="#"
                (click)="setView(view.id); $event.preventDefault()"
          ...
          <div class="px-4 py-4 border-t border-gray-700">
            <a 
                href="#"
                (click)="setView('profile'); $event.preventDefault()"
                class="flex items-center px-4 py-2 rounded-md transition-colors"
        ...
        <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6">
          ...
          <div class="flex items-center space-x-4">
            ...
            <div class="flex items-center space-x-2">
              <label for="user-select" class="text-sm font-medium text-gray-600">Viewing as:</label>
              <select id="user-select" (change)="changeUser($event)" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                @for (user of allUsers(); track user.id) {
                  <option [value]="user.id" [selected]="user.id === currentUser()?.id">{{ user.name }} ({{ user.role }})</option>
                }
              </select>
            </div>
            ...
            @switch (currentView()) {
              @case ('dashboard') {
                <app-dashboard 
                  ...
                  (payNow)="handlePayNow($event)"/>
              }
              @case ('admin') {
                <app-admin-dashboard />
              }
              @case ('new_request') {
                <app-service-request-form [client]="user" (requestSubmitted)="setView('dashboard')" />
              }
              @case ('schedule') {
                <app-schedule [user]="user" (viewDetails)="handleViewDetails($event)" />
              }
              @case ('search') {
