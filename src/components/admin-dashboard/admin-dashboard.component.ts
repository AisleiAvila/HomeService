...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, I18nPipe, DatePipe, CurrencyPipe],
...
export class AdminDashboardComponent {
  ...
  statusClass(status: ServiceStatus): string { const colors: Record<ServiceStatus, string> = { 'Pending': 'bg-yellow-100 text-yellow-800', 'Quoted': 'bg-cyan-100 text-cyan-800', 'Approved': 'bg-indigo-100 text-indigo-800', 'Scheduled': 'bg-teal-100 text-teal-800', 'Assigned': 'bg-blue-100 text-blue-800', 'In Progress': 'bg-purple-100 text-purple-800', 'Completed': 'bg-green-100 text-green-800', 'Cancelled': 'bg-gray-100 text-gray-800', }; return `px-2 py-1 text-xs font-semibold rounded-full ${colors[status]}`; }
}