...
export class AuthService {
  ...
  private async fetchAppUser(userId: string) {
    ...
    if (error && error.code !== 'PGRST116') { // PGRST116: "object not found" - this is expected on first login after signup
  ...
  private handleAuthError(error: AuthError | { message: string } | null, context: string) {
    if (error) {
  ...
  async login(email: string, password: string): Promise<AuthResponse> {
    const response = await this.supabase.client.auth.signInWithPassword({ email, password });
  ...
  async verifyOtp(email: string, token: string): Promise<AuthResponse> {
    const response = await this.supabase.client.auth.verifyOtp({ email, token, type: 'signup' });
  ...
  async resetPassword(email: string): Promise<void> {
    ...
    if (!error) {
        this.notificationService.addNotification('Password reset link sent. Please check your email.');
    }
  }
  ...
  async logout(): Promise<void> {
    ...
    if(!error) {
      this.appUser.set(null); // Explicitly clear user on logout
    }
  }
  ...
  async uploadAvatar(file: File): Promise<void> {
    ...
    // Use upsert to overwrite existing avatar for the user
