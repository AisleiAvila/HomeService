...
corsProxies.forEach((proxy, index) => {
  ...
  const request = https.get(proxyUrl, (res) => {
    console.log(`   Status: ${res.statusCode}`);
    console.log(`   Headers CORS: ${res.headers['access-control-allow-origin'] || 'Não encontrado'}`);
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 200) {
        try {
          ...
          if (parsed.contents) {
            ...
            try {
              ...
              if (Array.isArray(innerData) && innerData.length > 0) {
                ...
                console.log(`   📍 Primeira localidade: ${innerData[0].nome || innerData[0].localidade}`);
              }
            } catch (e) {
            ...
            console.log(`   📍 Primeira localidade: ${parsed[0].nome || parsed[0].localidade}`);
          } else {
          ...
          console.log(`   📝 Preview da resposta: ${data.substring(0, 200)}...`);
        }
      } else {
